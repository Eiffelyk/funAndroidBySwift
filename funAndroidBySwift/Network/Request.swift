...
extension Reactive where Base:MoyaProviderType{
    func request(_ target: Base.Target,
        ...
        return Observable<Response>.create{
...
extension Response{
    fileprivate func validate() -> ([String: Any]?, RequestError?){
        ...
        if let errCode = jsonDict["errorCode"] as? Int{
            if errCode == -1001 {
                return (nil, RequestError.loginInvalid)
            }
            
            if errCode != 0 {
                let message = (jsonDict["errorMsg"] as? String) ?? "服务器出错"
...
extension ObservableType where Element == Moya.Response {
    ...
    func validateSucess() -> Observable<Void> {

...
class RegisterModelView:ViewModel,ViewModelType {
    struct Input {
    ...
    func transform(input: Input) -> Output {
        let validate = Observable.combineLatest(input.userName, input.password, input.repPassword)
        ...
        return Output(
            validate: validate, registerSuccess: success
        )
    }
}

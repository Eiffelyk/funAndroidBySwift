...
class MineViewController: BaseViewController {
    struct ListItem {
        ...
        init(title: String, iconFont: Iconfont, viewController: @escaping @autoclosure () -> UIViewController, iconColor: UIColor? = nil){
            self.title = title
            self.icon = iconFont
    ...
    override func viewDidLoad() {
        ...
        headView.userNameBtn.rx.tap.subscribe(onNext: {[weak self] in
            guard let `self` = self, !AppState.share.loginUser.value.isLogin
                else {return}
